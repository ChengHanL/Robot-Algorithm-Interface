...
class Robot:
    ...
    def check_front(self):
        if self.bearing == Bearing.NORTH and self.validate(0, -1) and self.north_is_free():
            return True
        elif self.bearing == Bearing.EAST and self.validate(1, 0) and self.east_is_free():
            return True
        elif self.bearing == Bearing.SOUTH and self.validate(0, 1) and self.south_is_free():
            return True
        elif self.bearing == Bearing.WEST and self.validate(-1, 0) and self.west_is_free():
            return True
        else:
            return False
    ...
    def fastestPath(self, maze):
        ...
        for i in range(len(self.simulator.gp)):
            if self.simulator.gp[i][2] == 10:
                k.append([self.simulator.gp[i][0],
                          self.simulator.gp[i][1] - 4, 12, self.simulator.gp[i][3]])
            elif self.simulator.gp[i][2] == 11:
                k.append([self.simulator.gp[i][0] + 4,
                          self.simulator.gp[i][1], 13, self.simulator.gp[i][3]])
            elif self.simulator.gp[i][2] == 12:
                k.append([self.simulator.gp[i][0],
                          self.simulator.gp[i][1] + 4, 10, self.simulator.gp[i][3]])
            else:
                k.append([self.simulator.gp[i][0] - 4,
                          self.simulator.gp[i][1], 11, self.simulator.gp[i][3]])
    ...
    def hamiltonian_path_search(self, maze, target_obstacles):
        start = [18, 1, 10]
        end = [target_obstacles[0][1],
               target_obstacles[0][0],
               target_obstacles[0][2]]  # ending position
        ...
        for i in range(len(target_obstacles)):
            ...
            for j in range(len(self.simulator.robot_temp_movement)):
                move = [[tempStart[0]-1, tempStart[1]], [tempStart[0], tempStart[1]+1],
                        [tempStart[0]+1, tempStart[1]], [tempStart[0], tempStart[1]-1]]
                direction = [Bearing.NORTH, Bearing.EAST,
                ...
                for k in range(len(move)):
                    if move[k] in self.simulator.robot_temp_movement:
                        if self.bearing == direction[k]:
                            ...
                            tempStart = move[k]
                            self.simulator.robot_temp_movement.remove(
                                tempStart)
                            break
    ...
    def displayMovement(self):
        ...
        self.simulator.job = self.simulator.root.after(
            100, self.displayMovement)
