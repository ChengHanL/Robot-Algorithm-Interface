...
class Simulator:
    def __init__(self):
        ...
        self.canvas = Canvas(
            self.root, width=40 * config.map_size['width'], height=40 * config.map_size['height'])
        self.canvas.pack()

        self.control_panel = ttk.Frame(t, padding=(10, 10))
        self.control_panel.grid(row=0, column=1, sticky="snew")
        control_pane_window = ttk.Panedwindow(
            self.control_panel, orient=VERTICAL)
        ...
        hamiltonian_path_button = ttk.Button(action_pane, text='Hamiltonian Path Computation',
                                             command=self.hamiltonian_path, width=30)
        ...
        reset_button = ttk.Button(
            action_pane, text='Reset', command=self.reset)
        ...
        connect_button = ttk.Button(
            action_pane, text="Connect to RPI", command=self.communicate.connect, width=30)
        connect_button.grid(column=0, row=4, sticky='ew')
        disconnect_button = ttk.Button(
            action_pane, text="Disconnect to RPI", command=self.communicate.disconnect, width=30)
    ...
    def put_robot(self, x, y, bearing):
        if bearing == Bearing.NORTH:
            ...
            front_coor = (x * 40 + 40, y * 40 + 10,
                          x * 40 + 50, y * 40 + 20)
        elif bearing == Bearing.SOUTH_EAST:
            front_coor = (x * 40 + 35, y * 40 + 35,
                          x * 40 + 45, y * 40 + 45)
        elif bearing == Bearing.SOUTH:
            front_coor = (x * 40 + 15, y * 40 + 40,
                          x * 40 + 25, y * 40 + 50)
    ...
    def update_cell(self, x, y):
        def wall_radius(wall, wall_c):
            for i in range(3):
                for j in range(3):
                    if not (x - 1 + j < 0 or x + j > config.map_size['width'] or y - 1 + i < 0 or y - 1 + i >=
                            config.map_size['height']):
                        self.canvas.itemconfig(
                            config.map_cells_1[y - 1 + i][x - 1 + j], fill=wall_c)
                        if not map_sim[y - 1 + i][x - 1 + j] in [10, 11, 12,
                                                                 13]:  # Skip the obstacle itself to prevent changes
        ...
        if not config.map_cells_1[y][x]:
            config.map_cells_1[y][x] = self.canvas.create_rectangle(x * 40, y * 40, x * 40 + 40, y * 40 + 40,
                                                                    fill=color)
            config.map_cells_2[y][x] = self.canvas.create_text(x * 40 + 20, y * 40 + 20, text=direction, fill='black',
                                                               font='bold')
    ...
    def update_map(self, radius=2, full=False):
        if full:
            ...
            y_range = range(
                ...
                min(self.robot.y + radius,
                    config.map_size['height'] - 1) + 1
            )
            x_range = range(
                max(0, self.robot.x - radius),
                min(self.robot.x + radius,
                    config.map_size['width'] - 1) + 1
            )
